...
def handle_client(name: str, parametri: dict, client_socket: socket.socket):
    ...
    # Creazione dell'oggetto MQTT per il client
    ...
    while True:
        ...
        # Se il client ancora non e' stato inizializzato, procedere all'inizializzazione
        if not clientInitialised:
            # Connessione al broker MQTT facendo uso dell'access token comunicato
            clientMqtt.username_pw_set(data_json["ACCESS_TOKEN"])
            ...
            # Impoostare la flag in maniera da non inizializzare il client ancora
...
def platform_sender(parametri: dict, clientMqtt: mqtt.Client):
    ...
    # Continua l'esecuzione fino alla terminazione
    while True:
        ...
        if int(time.time() * 1000) - last_send > 60_000 * parametri["TEMPO_INVIO"] :
            ...
            try: # Effettua l'invio
               outcome = clientMqtt.publish(parametri["TOPIC"], output_data)
               # Attendere l'invio del messaggio
               outcome.wait_for_publish(timeout=5)

            except Exception as e:
                print("Errore durante l'invio ", e)
                break # Uscire dal ciclo di esecuzione del thread
